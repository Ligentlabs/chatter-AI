...
export class KeyVaultsGateKeeper {
  ...
  static initWithEnvKey = async () => {
    const { KEY_VAULTS_SECRET_KEY } = getServerDBConfig();
    if (!KEY_VAULTS_SECRET_KEY) throw new Error('KEY_VAULTS_SECRET_KEY is not set');

    const rawKey = Buffer.from(KEY_VAULTS_SECRET_KEY, 'base64'); // 确保密钥是32字节（256位）
