...
describe('KeyVaultsGateKeeper', () => {
  ...
  beforeEach(async () => {
    process.env.KEY_VAULTS_SECRET_KEY = 'Q10pwdq00KXUu9R+c8A8p4PSlIRWi7KwgUophBtkHVk=';
  ...
  it('should throw an error if KEY_VAULTS_SECRET_KEY is not set', async () => {
    // 将 KEY_VAULTS_SECRET_KEY 设为 undefined
    const originalSecretKey = process.env.KEY_VAULTS_SECRET_KEY;
    process.env.KEY_VAULTS_SECRET_KEY = '';

    // 断言在 KEY_VAULTS_SECRET_KEY 未设置时会抛出错误
    try {
      await KeyVaultsGateKeeper.initWithEnvKey();
    } catch (e) {
      expect(e).toEqual(Error('KEY_VAULTS_SECRET_KEY is not set'));
    }

    // 恢复 KEY_VAULTS_SECRET_KEY 的原始值
    process.env.KEY_VAULTS_SECRET_KEY = originalSecretKey;
  });
});
