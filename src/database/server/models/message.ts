...
import { ChatMessage, ChatTTS, ChatToolPayload } from '@/types/message';
...
export class MessageModel {
  ...
  async query({
    ...
    const result = await serverDB
      .select({
        ...
        ttsId: messageTTS.id,
        tts: {
          contentMd5: messageTTS.contentMd5,
          file: messageTTS.fileId,
          voice: messageTTS.voice,
        },
        /* eslint-enable */
      })
      .from(messages)
  ...
  async queryBySessionId(sessionId?: string): Promise<MessageItem[]> {
