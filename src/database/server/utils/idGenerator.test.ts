...
describe('idGenerator', () => {
  it('should generate an ID with the correct prefix and length', () => {
    ...
    expect(fileId).toMatch(/^file_[a-zA-Z0-9]{8}$/);

    const messageId = idGenerator('messages');
    expect(messageId).toMatch(/^msg_[a-zA-Z0-9]{8}$/);

    const pluginId = idGenerator('plugins');
    expect(pluginId).toMatch(/^plg_[a-zA-Z0-9]{8}$/);

    const sessionGroupId = idGenerator('sessionGroups');
    expect(sessionGroupId).toMatch(/^sg_[a-zA-Z0-9]{8}$/);

    const sessionId = idGenerator('sessions');
    expect(sessionId).toMatch(/^ssn_[a-zA-Z0-9]{8}$/);

    const topicId = idGenerator('topics');
    expect(topicId).toMatch(/^tpc_[a-zA-Z0-9]{8}$/);
